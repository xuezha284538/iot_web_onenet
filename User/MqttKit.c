...
uint8 MQTT_UnPacketRecv(uint8 *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		if(strstr((int8 *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	// 发送命令的路由
			status = MQTT_PKT_CMD;
		else
						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	// qos等级--主要针对PUBLISH（发布动态）消息的，保证信息数据的传递-----------------------------
	...
	// 需要保留PUBLISH（发布动态）消息，表示服务器需要保留最后一次发布的信息，当新的订阅者出现，就把信息发送给它，那么这个新的订阅者在连接之前的信息就都收到了
	if(will_retain)
	{
...
uint1 MQTT_PacketSaveData(const int8 *devid, int16 send_len, int8 *type_bin_head, uint8 type, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, "$sys/K3GGK1Sj7Q/stm/thing/property/post", NULL, send_len, MQTT_QOS_LEVEL0, 0, 1, mqttPacket) == 0)  // Publish消息
	{
...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
	...
	int8 *dataPtr = strchr((int8 *)rev_data + 6, '/');	// 前6个是头部信息
	...
	*cmdid = (int8 *)MQTT_MallocBuffer(37);				// cmdid固定36字节，多一个字符串结束位
	if(*cmdid == NULL)
		return 2;
	...
	*req = (int8 *)MQTT_MallocBuffer(*req_len + 1);		// 申请命令长度+1
	if(*req == NULL)
	{
	...
	*topic_len = (uint16)msgPtr[0] << 8 | msgPtr[1];
	...
	switch(*qos)
	{
		case MQTT_QOS_LEVEL0:									// qos0 have no packet identifier
			...
			*topic = MQTT_MallocBuffer(*topic_len + 1);			// 为topic申请内存
			if(*topic == NULL)
				return 255;
			...
			*payload = MQTT_MallocBuffer(*payload_len + 1);
			...
			{
				MQTT_FreeBuffer(*topic);						// 需要把topic的内存释放掉
				return 255;
			}
		...
		case MQTT_QOS_LEVEL2:
			...
			*payload = MQTT_MallocBuffer(*payload_len + 1);		// 为payload申请内存
			if(*payload == NULL)								// 申请失败
			{
				MQTT_FreeBuffer(*topic);						// 需要把topic的内存释放掉
				return 255;
